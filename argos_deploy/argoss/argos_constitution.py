"""
argoss/argos_constitution.py — Python реализация конституции ARGOS
================================================================
"""
...
class ArgosConstitution:
    """Загрузка и валидация конституции из YAML."""

    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.mode = ArgosMode(config.get("modes", {}).get("default", "normal"))

    def _threshold(self, section: str, key: str, default: float) -> float:
        return float(
            self.config.get("resources", {}).get(section, {}).get(key, default)
        )
    ...
    def validate_channels(self, channels: ChannelState) -> ConstitutionDecision:
        require_alive = self.config.get("channels", {}).get(
            "require_at_least_one_alive", True
        )
    ...
    def validate_patch(self, patch: PatchPlan) -> ConstitutionDecision:
        ...
        if patch.touches_live_core and not patch_cfg.get("live_patch_allowed", False):
            return ConstitutionDecision(
                allowed=False,
                reason="Live patch ядра запрещён конституцией",
        ...
        required_checks = [
            ...
            (
                patch_cfg.get("require_healthcheck", True),
                patch.has_healthcheck,
                "Нет health-check",
            ),
            (
                patch_cfg.get("require_rollback_plan", True),
                patch.has_rollback_plan,
                "Нет rollback plan",
            ),
        ]
    ...
    def validate_shell(self, command: str) -> ConstitutionDecision:
        ...
        if not shell_cfg.get("whitelist_only", True):
            return ConstitutionDecision(True, "Whitelist отключён")

        allowed_commands = set(shell_cfg.get("allowed_commands", []))
        if command not in allowed_commands:
            return ConstitutionDecision(
                allowed=False,
                reason=f"Shell-команда не в белом списке: {command}",
                require_owner_approval=True,
            )
    ...
    def validate_action(self, action: ActionRequest) -> ConstitutionDecision:
        ...
        if action.action_type in forbidden:
            return ConstitutionDecision(
                False, f"Действие запрещено: {action.action_type}"
            )
    ...
    def report(self, resources: ResourceState, channels: ChannelState) -> str:
        ...
        return (
            f"ARGOS Constitution Report\n"
            f"mode={mode.value}\n"
            f"cpu={resources.cpu_percent:.1f}% ram={resources.ram_percent:.1f}% disk={resources.disk_percent:.1f}%\n"
