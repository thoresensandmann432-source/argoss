...
class SelfHealingEngine:
    ...
    def auto_heal_file(self, path: str | Path, error_msg: str = "") -> str:
        ...
        if fixed is None:
            ...
            self._history.append(_HealRecord(
                timestamp=time.time(), file=str(p),
                error=error_msg, method="none", success=False,
            ))
        ...
        if not ok:
            ...
            self._history.append(_HealRecord(
                timestamp=time.time(), file=str(p),
                error=error_msg, method=method, success=False,
            ))
            if len(self._history) > _MAX_HISTORY:
                self._history = self._history[-_MAX_HISTORY:]
        ...
        self._history.append(_HealRecord(
            timestamp=time.time(), file=str(p),
            error=error_msg, method=method, success=True,
        ))
        if len(self._history) > _MAX_HISTORY:
            self._history = self._history[-_MAX_HISTORY:]
