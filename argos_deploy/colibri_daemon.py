...
try:
    from keystone import (
        ...
        KS_ARCH_AVR, KS_MODE_AVR32,
        KS_ARCH_MIPS, KS_MODE_MIPS32,
        KsError,
    )
    HAVE_KS = True
except ImportError:
...
if HAVE_CS:
    _CS_ARCHS = {
        ...
        "mips":      (cs_mod.CS_ARCH_MIPS,
                      cs_mod.CS_MODE_MIPS32 | cs_mod.CS_MODE_BIG_ENDIAN),
    }
    # AVR удалён в Capstone 5.0
...
class ColibriAsmEngine:
    ...
    def assemble(self, source: str, arch: str | None = None) -> dict:
        ...
        out: dict = {"ok": False, "arch": arch, "hex": "",
                     "bytes": b"", "count": 0, "listing": "", "error": ""}
    ...
    # ── Дизассемблирование ────────────────────────────────────────────────
    def disassemble(self, code: bytes, arch: str | None = None,
                    base_addr: int = 0) -> str:
        ...
        try:
            ...
            for ins in md.disasm(code, base_addr):
                ...
                lines.append(
                    f"  0x{ins.address:08x}:  {hx:<24}  {ins.mnemonic} {ins.op_str}"
                )
    ...
    def watch_file(self, path: str, arch: str | None = None,
                   on_result=None) -> str:
        """
            ...
            try:
                ...
                if mtime != last_mtime:
                    ...
                    log.info(
                        ...
                        fp.name, result["ok"], len(result.get("bytes", b"")),
                    )
                    if on_result:
                        try:
                            on_result(result)
