...
class _GPIOBackend:
    ...
    def input(self, pin: int) -> int: return 0
    def cleanup(self): ...
...
class _OpiGPIOBackend(_GPIOBackend):
    ...
    def output(self, pin, value): self._g.output(pin, value)
    def input(self, pin) -> int: return self._g.input(pin)
    def cleanup(self): self._g.cleanup()
    def name(self): return "OPi.GPIO"
...
class _GpiodBackend(_GPIOBackend):
    ...
    def _sn(self, pin): return OPI_ONE_PIN_MAP.get(pin, {}).get("sysfs", pin)
    def setup(self, pin, mode):
    ...
    def output(self, pin, value):
        if pin in self._lines: self._lines[pin].set_value(value)
    def input(self, pin) -> int:
        return self._lines[pin].get_value() if pin in self._lines else 0
    def cleanup(self):
        for l in self._lines.values(): l.release()
        self._lines.clear()
    def name(self): return "gpiod"
...
class _SysfsBackend(_GPIOBackend):
    ...
    def setup(self, pin, mode):
        ...
        if not gdir.exists():
            try: (self._BASE / "export").write_text(str(sn)); time.sleep(0.05)
            except Exception: pass
        try: (gdir / "direction").write_text("out" if mode == "OUT" else "in")
        except Exception as e: log.debug("sysfs setup %d: %s", pin, e)
    def output(self, pin, value):
        try: (self._BASE / f"gpio{self._sn(pin)}" / "value").write_text(str(value))
        except Exception as e: log.debug("sysfs out %d: %s", pin, e)
    def input(self, pin) -> int:
        try: return int((self._BASE / f"gpio{self._sn(pin)}" / "value").read_text().strip())
        except Exception: return 0
    def name(self): return "sysfs"


def _create_gpio_backend(preferred: str = "auto") -> _GPIOBackend:
    order = {"opi": ["opi","gpiod","sysfs"], "gpiod": ["gpiod","opi","sysfs"]}.get(preferred, ["opi","gpiod","sysfs"])
    for b in order:
        try:
            obj = _OpiGPIOBackend() if b == "opi" else _GpiodBackend() if b == "gpiod" else None
            if obj:
                log.info("GPIO backend: %s", obj.name()); return obj
        except Exception as e:
            log.debug("GPIO %s недоступен: %s", b, e)
    log.info("GPIO backend: sysfs"); return _SysfsBackend()
...
class OrangePiBridge:
    ...
    def __init__(self, core=None):
        ...
        log.info("OrangePiBridge init (I2C=bus%d UART=%s RS485=%s)",
                 self._i2c_bus, self._uart_port, self._rs485_port)
    ...
    def gpio_out(self, pin: int, value: int) -> str:
        try:
            ...
            if self._gpio_states.get(pin) != "OUT":
                g.setup(pin, "OUT"); self._gpio_states[pin] = "OUT"
            g.output(pin, value)
    ...
    def gpio_in(self, pin: int) -> str:
        try:
            ...
            if self._gpio_states.get(pin) != "IN":
                g.setup(pin, "IN"); self._gpio_states[pin] = "IN"
            val = g.input(pin)
            n = OPI_ONE_PIN_MAP.get(pin, {}).get("name", f"P{pin}")
    ...
    def gpio_status(self) -> str:
        ...
        lines = [f"🔌 GPIO Orange Pi One — бэкенд: {backend}",
    ...
    def pin_map(self) -> str:
        lines = ["📋 ORANGE PI ONE — 40-ПИН ХЕДЕР:\n",
    ...
    def i2c_scan(self) -> str:
        try:
            ...
            return (f"❌ I2C bus {self._i2c_bus} недоступен: {e}\n"
                    f"  Включить: sudo armbian-config → System → Hardware → i2c0/i2c1")

        known = {
            0x3C: "SSD1306 OLED",        0x3D: "SSD1306 OLED (alt)",
            0x40: "INA219 ток/напряж.",   0x48: "ADS1115 АЦП",
            0x68: "MPU6050 / DS3231 RTC", 0x69: "MPU6050 (alt)",
            0x76: "BME280/BMP280",        0x77: "BME280/BMP280 (alt)",
            0x60: "SI1145 UV/свет",       0x70: "PCA9548 мультиплексор",
        }
        found = []
    ...
    def read_bmp280(self, addr: int = 0x76) -> str:
        ...
        try:
            ...
            T2 = (calib[3] << 8 | calib[2]); T2 = T2 - 65536 if T2 > 32767 else T2
            T3 = (calib[5] << 8 | calib[4]); T3 = T3 - 65536 if T3 > 32767 else T3
            bus.write_byte_data(addr, 0xF4, 0x25); time.sleep(0.1)
    ...
    def _rs485_ensure(self):
        if self._rs485 is None or not self._rs485.is_open:
            ...
            self._rs485 = serial.Serial(
                self._rs485_port, self._rs485_baud,
    ...
    def modbus_read(self, slave: int, reg: int, count: int = 1) -> str:
        try:
            ...
            ser.setRTS(True); time.sleep(0.002)
    ...
    def modbus_write(self, slave: int, reg: int, value: int) -> str:
        try:
            ...
            ser.setRTS(False)
            return (f"✅ Modbus slave {slave} reg {reg} ← {value}" if ok
    ...
    def rs485_raw(self, data: bytes) -> str:
        try:
            ser = self._rs485_ensure()
            ser.setRTS(True); ser.write(data); ser.flush()
            time.sleep(0.005); ser.setRTS(False); time.sleep(0.05)
            resp = ser.read(64)
            return (f"📟 RS-485: {len(data)}B → {len(resp)}B  HEX: {resp.hex()}" if resp
    ...
    def spi_transfer(self, data: List[int], speed: int = 500000) -> str:
        try:
            ...
            self._spi.max_speed_hz = speed; self._spi.mode = 0
    ...
    def status(self) -> str:
        ...
        try: gpio_n = self._gpio_ensure().name()
        except Exception: pass
