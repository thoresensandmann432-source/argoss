"""
argos_multimodel_patch.py
Патч мультимодельного режима ARGOS:
  - Несколько локальных моделей Ollama параллельно
  - Одна облачная модель
  - ARGOS использует все запущенные модели одновременно
  - Специализация: быстрая модель для простых задач, умная для сложных

Запуск: python argos_multimodel_patch.py /путь/к/SiGtRiP
"""
import os
import sys
...
class OllamaInstance:
    ...
    def __init__(self, model: str, host: str = "http://localhost:11434"):
        self.model = model
        self.host = host
        self.profile = MODEL_PROFILES.get(model.split(":")[0], {
            "speed": 3, "quality": 3, "ctx": 4096, "role": "balanced"
        })
...
mm_path = REPO / "src" / "multi_model.py"
mm_path.write_text(MULTI_MODEL, encoding="utf-8")
print(f"✅ {mm_path}")
...
# Дополнительные модели (через запятую)
# Пример: OLLAMA_MODELS=llama3,mistral:7b,codellama:13b
OLLAMA_MODELS=

# Быстрая модель для простых команд
OLLAMA_FAST_MODEL=tinyllama

# Режим consensus (опрашивать N моделей параллельно)
ARGOS_CONSENSUS_MODE=off
ARGOS_CONSENSUS_N=2

# Основной облачный провайдер (gemini/groq/deepseek/xai)
ARGOS_PRIMARY_CLOUD=gemini
"""

env_path = REPO / ".env.example"
existing = env_path.read_text(encoding="utf-8") if env_path.exists() else ""
if "Мультимодельный" not in existing:
...
check_path = REPO / "check_models.py"
check_path.write_text(CHECK, encoding="utf-8")
print(f"✅ {check_path}")

print("""
╔══════════════════════════════════════════════════════════╗
║  Мультимодельный патч применён!                           ║
╚══════════════════════════════════════════════════════════╝

Рекомендуемые модели для телефона (мало RAM):
  ollama pull tinyllama      # 637MB — быстрая
  ollama pull gemma2:2b      # 1.6GB — умная лёгкая
  ollama pull llama3.2:3b    # 2GB   — сбалансированная

Для ПК (8GB+ RAM):
  ollama pull llama3:8b      # 4.7GB
  ollama pull mistral:7b     # 4.1GB
  ollama pull codellama:13b  # 7.4GB — для кода

В .env добавь:
  OLLAMA_MODELS=tinyllama,gemma2:2b
  OLLAMA_FAST_MODEL=tinyllama
  ARGOS_PRIMARY_CLOUD=gemini
