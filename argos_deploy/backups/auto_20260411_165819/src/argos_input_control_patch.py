...
REPO = Path(sys.argv[1]) if (len(sys.argv) > 1 and __name__ == '__main__') else Path(__file__).parent.parent
...
for f in ["src/input_control.py", "src/core.py"]:
    r = sp.run(["python3", "-m", "py_compile", str(REPO / f)], capture_output=True, text=True)
    if r.returncode == 0:
