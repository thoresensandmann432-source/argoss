...
def bump(version: str = _SENTINEL, part: str = "patch", *, dry_run=_SENTINEL) -> str:  # type: ignore[assignment]
    """Двойной интерфейс:
    bump("1.0.0")          -> "1.0.1"  (поднять версию строки)
    bump(dry_run=True/False) -> None   (применить к файлам проекта)
    """
    if dry_run is not _SENTINEL:
        run_bump(dry_run=bool(dry_run))
...
def run_bump(dry_run: bool = False) -> None:
    import sys
    # Пересчитываем ROOT динамически — чтобы тесты могли monkey-patch __file__
    _mod = sys.modules[__name__]
    _root = Path(getattr(_mod, "__file__", __file__)).parent.resolve()
    _pyproject = _root / "pyproject.toml"
    _build_py  = _root / "build.py"
    _readme    = _root / "README.md"
    # Вычисляем актуальные версии из реального файла
    _text = _pyproject.read_text(encoding="utf-8") if _pyproject.exists() else ""
    _m = _VERSION_RE.search(_text)
    _old = _m.group(0).split('"')[1] if _m else OLD
    _new = bump(_old, "patch")
    _targets = [
        (_pyproject,            _old, _new),
        (_build_py,             _old, _new),
        (_readme,               _old, _new),
        (_root / "manifest.json",  _old, _new),
        (_root / "manifest.yaml",  _old, _new),
    ]
    print(f"\n📦 Текущая версия : {_old}")
    print(f"🚀 Новая версия   : {_new}\n")
    for path, old_ver, new_ver in _targets:
