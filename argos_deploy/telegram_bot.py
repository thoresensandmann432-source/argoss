...
try:
    from arc_play import play_game as play_ls20
except ImportError:
    play_ls20 = None  # type: ignore
...
# Max bytes to download and display from a document
MAX_FILE_READ_BYTES = 1 * 1024 * 1024  # 1 MB
# Max characters to display from a text file
MAX_FILE_DISPLAY_CHARS = 3000

# ── Voice chat tracking ────────────────────────────────────────────────────────
_voice_chats: set[int] = set()

# ── Cached ArgosCore instance ─────────────────────────────────────────────────
_argos_core = None


def _get_argos_core():
    """Return a cached ArgosAbsolute instance (loads main.py on first call)."""
    global _argos_core
    if _argos_core is not None:
        return _argos_core
    try:
        ...
        log.warning("_get_argos_core: %s", e)
        _argos_core = None
    return _argos_core


def _tts_to_bytes(text: str, lang: str = "ru") -> bytes | None:
    """Convert text to MP3 bytes using gTTS. Returns None if gTTS is unavailable."""
    try:
    ...
    try:
        truncated = text[:500]
        buf = io.BytesIO()
        gTTS(text=truncated, lang=lang).write_to_fp(buf)
        buf.seek(0)
        return buf.read()
    except Exception as e:
        log.warning("_tts_to_bytes error: %s", e)
        return None


def _read_file_content(raw: bytes, filename: str) -> str:
    """Decode raw bytes from a document and return a readable text preview."""
    if not raw:
        return "📄 Содержимое файла: (пусто)"
    # Detect binary data
    if b"\x00" in raw:
        size_kb = len(raw) / 1024
        return f"🗜 Получены бинарные данные ({size_kb:.1f} КБ)"
...
async def cmd_argos(message: Message):
    ...
    try:
        ...
        if core is None:
            raise RuntimeError("ArgosCore недоступен")
...
async def cmd_voice_on(message: Message):
    """Включить голосовой ответ для текущего чата."""
    _voice_chats.add(message.chat.id)
    await message.answer("🔊 Голосовой ответ включён для этого чата.")


@dp.message(Command("voice_off"))
async def cmd_voice_off(message: Message):
    """Выключить голосовой ответ для текущего чата."""
    _voice_chats.discard(message.chat.id)
    await message.answer("🔇 Голосовой ответ выключён для этого чата.")
...
async def handle_document(message: Message):
    """Handle incoming document files: download, decode, and reply with content."""
    doc = message.document
    fname = getattr(doc, "file_name", None) or "неизвестный_файл"
    ...
    await save_message(
        ...
        full_name=getattr(user, "full_name", "Unknown") or "Unknown",
        text=f"[ФАЙЛ: {fname}, {fsize} байт]",
    )

    if fsize > MAX_FILE_READ_BYTES:
        await message.answer(
            f"⚠️ Файл '{fname}' слишком большой ({fsize / 1024 / 1024:.1f} МБ).\n"
            f"Максимум: {MAX_FILE_READ_BYTES // 1024 // 1024} МБ."
        )
        return
    ...
    reply = _read_file_content(raw, fname)
    # Trim to Telegram max
    for chunk_start in range(0, len(reply), TELEGRAM_MAX_MESSAGE_LENGTH):
        await message.answer(reply[chunk_start: chunk_start + TELEGRAM_MAX_MESSAGE_LENGTH])
